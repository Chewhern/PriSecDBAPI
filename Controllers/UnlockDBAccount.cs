...
namespace PriSecDBAPI.Controllers
{
    ...
    [ApiController]
    public class UnlockDBAccount : ControllerBase
    {
        ...
        [HttpPost]
        public String UnlockDBAccountFunction(LockDBAccountModel MyLockModel) 
        {
            String Status = "";
                                    ...
                                    if (Count == 1) 
                                    {
                                        ...
                                        if (TimeDifference.TotalMinutes < 8) 
                                        {
                                            if(DBUserName.CompareTo("Chew")==0 || DBUserName.CompareTo("Lynas")==0 || DBUserName.CompareTo("Hern")==0 || DBUserName.CompareTo("Chrono") == 0) 
                                            {
                                                ...
                                                MyGeneralGCHandle.Free();
                                                MyGeneralGCHandle = GCHandle.Alloc(MyLockModel.SealedSessionID, GCHandleType.Pinned);
                                                SodiumSecureMemory.MemZero(MyGeneralGCHandle.AddrOfPinnedObject(), MyLockModel.SealedSessionID.Length);
                                                MyGeneralGCHandle.Free();
                                            ...
                                            MySQLGeneralQuery = new MySqlCommand();
                                            MySQLGeneralQuery.CommandText = "ALTER USER " + "'" + DBUserName + "'" + "@'localhost' ACCOUNT UNLOCK";
                                            ...
                                            SodiumSecureMemory.MemZero(MyGeneralGCHandle.AddrOfPinnedObject(), MyLockModel.SealedSessionID.Length);
                                            MyGeneralGCHandle.Free();
                                            Status = "Success: The specified DB user account have been unlocked";
                                            return Status;
                                        }
                                        else 
                                        {
